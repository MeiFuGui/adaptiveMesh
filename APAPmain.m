...
C1 = 2; % Resolution/grid-size for the mapping function in MDLT (C1 x C2).MDLT中映射函数的分辨率/网格大小
C2 = 2;
            ...
            path4 =    'F:\imagesData\LPC-01_library\result\APAP_result.jpg';   %APAP图像拼接结果
            t1 = clock; 
            %-------------
            % Read images.
            %-------------
...
%linear_hom(linear_hom == 0) = 255;%黑变白
fprintf('done (%fs)\n',toc);
% figure;
%imshow(linear_hom);
...
warped_img2(isnan(warped_img2)) = 255; % 将未映射的区域填充为白色
warped_img2 = reshape(uint8(warped_img2),size(warped_img2,1),size(warped_img2,2)/3,3);
fprintf('done (%fs)\n',toc);
figure;
imshow(warped_img2);
