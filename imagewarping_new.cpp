...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    img2m = mxGetM(prhs[2]);//图像高度
    img2n = mxGetN(prhs[2]) / 3;//图像宽度 /* It is an RGB image, which means that for MATLAB it is an Mx(Nx3) image. So, we have  
    ...
    if (nrhs == 7){ /* Get input parameters for MDLT warping. */
        X     = mxGetPr(prhs[5]);//prhs[5]：第6个输入参数
        Y     = mxGetPr(prhs[6]);//prhs[6]：第7个输入参数

                
        xn    = mxGetN(prhs[5]);    /*length of X*///获取 prhs[5] 数组的列数
    ...
    if (nrhs == 5){ /*We stitch Global Homography (DLT). */
        ...
        for(i=0;i<canvm;i++)//canvm图像的高度遍历//i表示当前行
        {
            for(j=0;j<canvn;j++)//canvn图像的宽度遍历//j表示当前列
            {   //对于画布上的每个像素，计算其在源图像中对应的像素位置（posa（行） 和 posb（列））。
                //如果计算出的坐标在源图像的有效范围内，则将源图像的像素值复制到画布。
...
//     }
        ...
        for(xinx=0; xinx < xn; xinx++)
            for(yinx=0; yinx < yn; yinx++)
                for(int i = X[xinx];i<X[xinx]+canvm/100;i++)
                    for(int j = Y[yinx];j<Y[yinx]+canvn/100;j++){
                        inx = yinx + xinx*yn;//inx是网格点索引
...
//         for(int inx = 0;inx < xn*yn;inx++){
//             for(int i=X[inx];i < X[inx]+canvm/100;i++){
//                 for(int j=Y[inx];j < Y[inx]+canvn/100;j++){
//                     /* Get projective transformation of this grid point and warp the current pixel with it. */
//                     posa = round(((H[inx+(Hm*0)] * (j-off[0]+1)) + (H[inx+(Hm*3)] * (i-off[1]+1)) + H[inx+(Hm*6)]) / ((H[inx+(Hm*2)] * (j-off[0]+1)) + (H[inx+(Hm*5)] * (i-off[1]+1)) + H[inx+(Hm*8)]));
//                     posb = round(((H[inx+(Hm*1)] * (j-off[0]+1)) + (H[inx+(Hm*4)] * (i-off[1]+1)) + H[inx+(Hm*7)]) / ((H[inx+(Hm*2)] * (j-off[0]+1)) + (H[inx+(Hm*5)] * (i-off[1]+1)) + H[inx+(Hm*8)]));
//                      if ((posa > 1)&&(posa < img2n)&&(posb>1)&&(posb<img2m)){
...
//         for(int i=0;i<canvm;i++){
//             for(int j=0;j<canvn;j++){
//                 
//             }
//         }
        for(int inx = 0;inx < xn;inx++){
            for(int i=X[inx];i < X[inx]+xn_len[inx];i++){
                for(int j=Y[inx];j < Y[inx]+yn_len[inx];j++){
                    /* Get projective transformation of this grid point and warp the current pixel with it. */
                    posa = round(((H[inx+(Hm*0)] * (j-off[0]+1)) + (H[inx+(Hm*3)] * (i-off[1]+1)) + H[inx+(Hm*6)]) / ((H[inx+(Hm*2)] * (j-off[0]+1)) + (H[inx+(Hm*5)] * (i-off[1]+1)) + H[inx+(Hm*8)]));
                    posb = round(((H[inx+(Hm*1)] * (j-off[0]+1)) + (H[inx+(Hm*4)] * (i-off[1]+1)) + H[inx+(Hm*7)]) / ((H[inx+(Hm*2)] * (j-off[0]+1)) + (H[inx+(Hm*5)] * (i-off[1]+1)) + H[inx+(Hm*8)]));
                     if ((posa > 1)&&(posa < img2n)&&(posb>1)&&(posb<img2m)){
