...
Hmdlt = zeros(size(Mv_new,1),9);%初始化Hmdlt矩阵，用于存储每个网格点的转换矩阵
parfor i=1:size(Mv_new,1)%使用 parfor 循环实现并行计算，处理每个网格点的变换。
    
    % Obtain kernel
    Gki = exp(-pdist2(Mv_new(i,1:2),Kp)./sigma^2);%pdist2 函数计算当前网格点与已知点 Kp 的距离，随后应用指数函数生成权重 Gki。
